...
public abstract class EntityRendererMixin {
    ...
    private double disableDistanceLimit(double constant){
        if (this.mc.thePlayer.capabilities.isCreativeMode) {
            ...
            if (heldItem != null) {
                ...
                if (id == Item.swordWood.itemID || (heldItem.getTagCompound() != null && heldItem.getTagCompound().hasKey("BuildingParams")))
                    return 999999;
            }
    ...
    private boolean disableRedNightVis(EntityLivingBase instance, Potion par1Potion){
        if (this.mc.thePlayer.capabilities.isCreativeMode&&(boolean) VANILLA_NIGHTVIS.getValue()){
            return false;
    ...
    private int[] manageNightvisionColor(int[] par1ArrayOfInteger) {
        if (this.mc.thePlayer.capabilities.isCreativeMode&&this.mc.thePlayer.isPotionActive(Potion.nightVision)&&(boolean) VANILLA_NIGHTVIS.getValue()) {
